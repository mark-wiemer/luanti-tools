...
noise = offset + scale _ (octave1 +
octave2 _ persistence +
octave3 _ persistence ^ 2 +
octave4 _ persistence ^ 3 +
...)
...
noise = offset + scale _ (abs(octave1) +
abs(octave2) _ persistence +
abs(octave3) _ persistence ^ 2 +
abs(octave4) _ persistence ^ 3 +
...)
...
The coordinates are _inclusive_, like most other things in Minetest.
...
field by calling `minetest.set_gen_notify()` _before_ map generation happens.
